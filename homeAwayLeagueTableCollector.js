...
// @match        https://footystats.org/vn/*/*/over-25-goals-table
// @match        https://footystats.org/vn/*/*/xg
...
;(function () {
  ...
  try {
    ...
    function getCleanTeamName(cell) {
      const link = cell?.querySelector?.('a')
      if (!link) return getCellText(cell)

      return (
        getDirectText(link) ||
        getCellText(link)
          .replace(getCellText(link.querySelector('.hover-modal-content')), '')
          .trim()
      )
    }

    function getTeamIdFromHref(href) {
    ...
    function findNearestPreviousH2(node, root) {
      ...
      while (current && current !== root) {
        ...
        while (sibling) {
          ...
          const nested = sibling.querySelector?.('h2')
    ...
    function extractRowsFromTable(table) {
      ...
      return bodyRows
        .map((tr) => {
          ...
          const teamLink = dataCells[0]?.querySelector('a')
          const directUrl = teamLink?.getAttribute('href')?.trim() || ''
          const teamId = getTeamIdFromHref(directUrl)
          const rowKey = teamId || directUrl
    ...
    function extractHomeAwayLeagueTableData(doc = document) {
      ...
      sections.forEach(({ title, table }, sectionIndex) => {
        ...
        extractRowsFromTable(table).forEach(
          ({ teamId, directUrl, rowKey, values }) => {
            if (!rowsByTeamId.has(rowKey)) {
              const row = Array(headers.length - 1).fill('')
              row._teamId = teamId
              row._directUrl = directUrl
              rowsByTeamId.set(rowKey, row)
            }

            const row = rowsByTeamId.get(rowKey)
            if (!row._teamId && teamId) row._teamId = teamId
            if (!row._directUrl && directUrl) row._directUrl = directUrl
      ...
      const rows = Array.from(rowsByTeamId.values()).map((values) => [
        values._teamId || values._directUrl || '',
        ...values,
      ])
    ...
    function extractOver25GoalsTableData(doc = document) {
      ...
      tables.forEach((table, sectionIndex) => {
        const title = getCellText(findNearestPreviousH2(table, doc.body))
        const sectionHeaders = getTableHeaders(table)
        const sectionStartIndex = headers.length - 1
        headers.push(...sectionHeaders)
        sectionHeaders.forEach((header, columnIndex) => {
        ...
        bodyRows.forEach((tr) => {
          ...
          const values = [
            getCleanTeamName(teamCell),
            ...dataCells.slice(1).map(getCellText),
          ]
          values.forEach((value, index) => {
            row[sectionStartIndex + index] = value
          })
    ...
    function extractXgData(doc = document) {
      ...
      if (!lists.length) return [['Status'], ['No xG ui-list found.']]
      ...
      lists.forEach((list, sectionIndex) => {
        const title = getDirectText(list.querySelector('li.list-title'))
        const sectionHeaders = ['Đội', 'Chỉ số']
        const sectionStartIndex = headers.length - 1
        headers.push(...sectionHeaders)
        sectionHeaders.forEach((header, columnIndex) => {
          columnMeta.push({
        ...
        Array.from(list.querySelectorAll('li.list-row')).forEach((item) => {
          const teamLink = item.querySelector('.main a[href]')
          const directUrl = teamLink?.getAttribute('href')?.trim() || ''
          const teamId = getTeamIdFromHref(directUrl)
          const rowKey = teamId || directUrl
    ...
    const PAGE_EXTRACTORS = [
      {
        name: 'home-away-league-table',
        match: (path) => path.includes('/home-away-league-table'),
        extract: extractHomeAwayLeagueTableData,
      },
      {
        name: 'over-25-goals-table',
        match: (path) => path.includes('/over-25-goals-table'),
        extract: extractOver25GoalsTableData,
      },
      {
        name: 'xg',
        match: (path) => path.endsWith('/xg'),
        extract: extractXgData,
      },
    ]

    function extractCurrentPageData(doc = document) {
      const path = window.location.pathname
      const extractor = PAGE_EXTRACTORS.find((item) => item.match(path))
      if (!extractor) return [['Status'], [`Unsupported page: ${path}`]]
